...
class SyncTaskTest {
    ...
    fun sdCardRootJobPropagatesDeletionsThroughTheAppTrash() = coreTest(timeoutMs = 25 * 60_000L) {
        ...
        val id = job.text("id")
        val firstTask = Api.runTask("sync.run", args("id" to id), timeoutMs = 600_000)
        val first = firstTask.resultObj()
        TaskReport.note("sd-root-job", "erster Lauf: ${first.textOrNull("summary")} ${firstTask.errors}")
        // Any error keeps the first baseline from being saved, so deletions could not follow.
        assertEquals("Fehler im ersten Lauf: ${firstTask.errors}", 0, first.int("errors"))
        ...
        val secondTask = Api.runTask("sync.run", args("id" to id), timeoutMs = 600_000)
        val second = secondTask.resultObj()
        TaskReport.note("sd-root-job", "zweiter Lauf: ${second.textOrNull("summary")} ${secondTask.errors}")
    ...
    fun mirroringTheSdCardRootOmitsTheAppTrash() = coreTest(timeoutMs = 25 * 60_000L) {
        ...
        val mirrorTask = Api.runTask("sync.mirror", args("source" to sd.path, "target" to target.absolutePath), timeoutMs = 600_000)
        val mirror = mirrorTask.resultObj()
        TaskReport.note("mirror", "$mirror ${mirrorTask.errors}")
        assertEquals("Spiegelfehler: ${mirrorTask.errors}", 0L, mirror.long("errors"))
