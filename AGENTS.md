...
- Before triggering release publication with a tag, workflow dispatch, or `release/vX.Y.Z` branch, verify `native/Cargo.toml`, `release-native/update-feed/version.txt`, the matching installer, all six desktop update-feed payloads plus the Android APK (`smart-explorer-android.apk`), and their `.sha256` files agree for the same version.
