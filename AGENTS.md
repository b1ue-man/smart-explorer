...
- For native app changes, always bump the patch version, build the Windows release artifacts, commit the version/artifact changes on `main`, push `main`, create the matching `vX.Y.Z` tag, push the tag, and verify the matching GitHub Release unless the user explicitly says not to or this is technically blocked.
- For release work, always build the release artifacts before calling the release done, then commit and push the release changes and artifacts to the configured remote, publish the matching GitHub Release, and verify it unless the user explicitly says not to or pushing/publishing is technically blocked.
- A complete release must have a GitHub Release on the repository's Releases page for the same `vX.Y.Z` as `native/Cargo.toml`, `release-native/update-feed/version.txt`, and the installer. Prefer the existing CI path: push the `vX.Y.Z` tag. If tag push is technically blocked, use an existing CI fallback: `workflow_dispatch` when available, or the documented `docs/RELEASING.md` fallback of pushing `release/vX.Y.Z`. Do not call the release done until the CI release job has succeeded and the GitHub Release is visible with the expected assets: Windows installer, Windows app/updater payloads and hashes, Linux app/updater payloads and hashes, `install-linux.sh`, context-menu DLL, share-server payloads, and `version.txt`. If publishing is blocked, report the exact trigger attempted, the failure, and the remaining release step.
...
- If tag-triggered CI fails after a tag was pushed, do not rewrite or move the published tag. Inspect the failed job logs, fix the cause on `main`, bump the native patch version again, rebuild local release artifacts, commit, push `main`, create the next `vX.Y.Z` tag, push it, and verify that new GitHub Release.
...
- Before triggering release publication with a tag, workflow dispatch, or `release/vX.Y.Z` branch, verify `native/Cargo.toml`, `release-native/update-feed/version.txt`, the matching installer, all four update-feed payloads, and their `.sha256` files agree for the same version.
...
- After modifying native source code, update the root graph from the repository root with PowerShell: `$env:GRAPHIFY_FORCE='1'; graphify extract native/src --out . --no-cluster; Remove-Item Env:\GRAPHIFY_FORCE -ErrorAction SilentlyContinue`, then `graphify cluster-only . --no-viz --no-label`. The force environment variable avoids a stale incremental manifest shrinking `graphify-out/graph.json` to only changed files while still writing to the root `graphify-out/`.
