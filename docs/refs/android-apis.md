...
## 11. Ergänzungen K0 (2026-09-25, Android-Projekt-Fundament)

Nachgeschlagen für `android/app` (Block K0); Quellen je Zeile.

| API | Fakt | Quelle |
|---|---|---|
| `Settings.Global.BOOT_COUNT` | `String`-Konstante (`"boot_count"`), API 24; Wert ist `int` = Anzahl der Starts seit API-24-Betrieb (seit Werksreset). Lesen: `Settings.Global.getInt(contentResolver, Settings.Global.BOOT_COUNT, -1)` (Overload mit Default wirft nicht). Dient als `bootMarker` der Init-Konfiguration (api.md §1). | learn.microsoft.com Mono-Spiegel `Settings.Global.BootCount` (`Register("BOOT_COUNT", ApiSince=24)`, „Type: int“), verlinkt developer.android.com `Settings.Global#BOOT_COUNT` |
| `Settings.Global.DEVICE_NAME` | `String`-Konstante, API 25, „The name of the device“; `Settings.Global.getString(cr, DEVICE_NAME)` kann `null` sein → Rückfall `Build.MODEL`. | learn.microsoft.com `Settings.Global.DeviceName` (`ApiSince=25`) |
| `StorageManager.registerStorageVolumeCallback(Executor, StorageManager.StorageVolumeCallback)` | API 30; `StorageVolumeCallback` ist eine Klasse mit `open fun onStateChanged(volume: StorageVolume)` (API 30), gerufen bei `StorageVolume#getState()`-Wechseln (mounted/unmounted …); danach `getStorageVolumes()` für den aktuellen Stand. | learn.microsoft.com `StorageManager.RegisterStorageVolumeCallback` / `StorageVolumeCallback.OnStateChanged` (Signatur-Deskriptoren, `ApiSince=30`) |
| `Context.getMainExecutor()` | API 28 – Executor für den Callback oben. | developer.android.com `Context` (Plattformstand; nicht separat abgerufen) |
| `PackageManager.getPackageInfo(String, PackageManager.PackageInfoFlags)` | API 33 (`PackageInfoFlags.of(long)`, `ApiSince=33`); darunter `getPackageInfo(String, Int)` (ab 33 deprecated). `PackageInfo.longVersionCode` API 28. | learn.microsoft.com `PackageManager.PackageInfoFlags.Of` |
| `Intent.setIdentifier(String?)` | API 29; nimmt an `filterEquals()` teil (nie am `IntentFilter`-Abgleich) → unterscheidet sonst gleiche `PendingIntent`s. | learn.microsoft.com `Intent.SetIdentifier` (`ApiSince=29`) |
| `data_extraction_rules.xml` | `<data-extraction-rules><cloud-backup>…</cloud-backup><device-transfer>…</device-transfer></data-extraction-rules>`, `<exclude domain="root|file|database|sharedpref|external|device_root|device_file|device_database|device_sharedpref" path="."/>`; nur `exclude` = alles andere eingeschlossen, daher `root`, `device_root`, `external` je mit `path="."` ausschließen. `allowBackup="false"` stoppt ab API 31 auf manchen Geräten die Gerät-zu-Gerät-Übertragung nicht – deshalb beides. | developer.android.com/identity/data/autobackup |
| `SystemBarStyle.auto(lightScrim: Int, darkScrim: Int, detectDarkMode: (Resources) -> Boolean = …)` | `androidx.activity`; `enableEdgeToEdge(statusBarStyle, navigationBarStyle)` darf erneut aufgerufen werden (z. B. wenn das App-Theme vom System abweicht). Standard-Scrims: hell `Color.argb(0xe6,0xFF,0xFF,0xFF)`, dunkel `Color.argb(0x80,0x1b,0x1b,0x1b)`. | androidx `activity/activity/src/main/java/androidx/activity/EdgeToEdge.kt` (androidx-main) |
| `androidx.lifecycle.compose.LocalLifecycleOwner`, `LifecycleEventEffect(event, lifecycleOwner = LocalLifecycleOwner.current, onEvent)` | `lifecycle-runtime-compose` (2.10.0 enthalten); `ON_DESTROY` nicht erlaubt. `androidx.compose.ui.platform.LocalLifecycleOwner` ist veraltet. | androidx `lifecycle/lifecycle-runtime-compose/src/commonMain/.../LifecycleEffect.kt`, `LocalLifecycleOwner.kt` |
| `currentWindowAdaptiveInfo().windowSizeClass` | `adaptive-android:1.2.0` hängt von `androidx.window:window-core` ≥ 1.4.0 ab; `WindowAdaptiveInfo.windowSizeClass` ist `androidx.window.core.layout.WindowSizeClass`, das in window-core 1.4.0 `isWidthAtLeastBreakpoint(Int)` und `WIDTH_DP_EXPANDED_LOWER_BOUND` (840) enthält. Die Klasse referenziert `ExperimentalMaterial3AdaptiveApi` → `@OptIn` defensiv setzen. | Gradle-Modul-Metadaten `adaptive-android-1.2.0.module`; Klassen in `window-core-android-1.4.0.aar` und `adaptive-android-1.2.0.aar` (dl.google.com) geprüft |
