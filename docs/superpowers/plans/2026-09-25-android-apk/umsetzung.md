...
| G4 Gesamtablauf Gerät | F3–F17, F19–F22 | Emulator (x86_64, SD-Karten-Image), `adb` gewährt Dateizugriff/Benachrichtigungen; instrumentierte Tests über die echte App: lokale Abläufe (neu, umbenennen, filtern, rekursiv mit Fenstern, kopieren, verschieben zwischen Volumes, Eigenschaften, Papierkorb je Volume + Wiederherstellen, ZIP), Ordnerindex, SFTP/WebDAV/FTP auf dem Runner (`10.0.2.2`), Sync interner Speicher↔SD-Karte inkl. Konflikt/Prüfen/Lösen/Zusammenführen (reines SFTP ersetzt keine Dateien, wie am Desktop), Remote-Bearbeitung mit Konflikt/Erzwingen über den Remote-Agent (zweiter SSH-Server), Überschreiben auf reinem SFTP abgewiesen, Job auf Volume-Wurzel mit Löschung (kein Papierkorb-Upload, fremde App-Ordner `Android/data|obb/*` als geschützte Auslassung), Spiegeln der Volume-Wurzel (Papierkorb ausgelassen), Rename-Fehlernummern je Volume protokolliert, `bg.catchUp` über `work-testing`-TestDriver (Bedingungen/Periode), Dauerbetrieb-Dienst + Benachrichtigung, Übertragung läuft nach Home-Taste weiter (UiAutomator), echter Neustart (`adb reboot`; nur das System-BOOT_COMPLETED erlaubt den Vordergrunddienst), Öffnen/Teilen (Espresso-Intents), Empfangen (`am start -a SEND` mit Deskriptor), Analyse, Duplikate, Update gegen lokalen Test-Feed | alle Tests grün; Abdeckungsliste: jede api.md-Methode aufgerufen außer ausdrücklich ausgenommen (`gdrive.signIn`: echtes Google-Konto nötig) |
...
- Remote-Bearbeitung/Sync-Konflikte auf reinem SFTP: SFTP-v3-Rename ersetzt nicht, das ist am Desktop Absicht (`docs/REMOTE_EDIT.md`) → `fs.uploadEdit` meldet vorab `replaceUnsupported`, die App bietet nur „Als Kopie hochladen“; Hinweis am Remote-Agent-Schalter. Tests: Konflikte zwischen zwei lokalen Volumes, Bearbeiten mit Konflikt/Erzwingen über den Remote-Agent auf einem zweiten SSH-Server.
- `bg.catchUp` direkt nach dem Einschalten: der Daemon beurteilte nach dem Schlaf-Intervall mit dem alten Schalterstand → nach dem Schlaf erneut prüfen.
- Share-Freigabe nicht im Status: ein vor dem Worker-Neuladen geleerter Snapshot überschrieb die eben gespeicherten Profile → Revision des Commits schützt 10 s lang.
