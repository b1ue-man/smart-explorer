...
| # | Item | State | Version |
|---|---|---|---|
| 21c | **Peer remote execution** — unrestricted argv/shell execution uses a separate authenticated Iroh Exec ALPN, exact default-deny per-device grants, at-most-once Start, binary streaming IPC, explicit active/history state, atomic revoke/cancel, Windows kill-on-close Job Objects, and Linux transient systemd cgroups. The installed static Linux release-feed CLI and release Share server pass the two-profile E2E using only prior CLI output, covering binary I/O, nonzero exit, timeout, output cap, Cancel, CLI death, worker `SIGKILL`, revoke, and post-revoke denial; Windows CI runs the native Job-Object self-test plus a real two-profile Share/Exec CLI lifecycle with remote `cmd.exe`. | ✅ | 0.5.132 |
