...
| 21 | **Device-to-device sharing** (Quick Share / AirDrop / own pairing) — **evaluated + refined**: see [`docs/SHARING_EVAL.md`](SHARING_EVAL.md). AirDrop ❌ on Windows even after the 2025 Quick-Share↔AirDrop interop (AWDL unpublished, no Windows API; Google's own app only); Quick Share ✅ same-LAN (ref `rquickshare`); **own paired share, server = router/STUN ONLY (no relay), bytes direct P2P ✅ recommended**, plus a **peer-agent Backend** (far Smart Explorer runs scans/filters/search, streams results). Awaiting your go + which paths. | next | decision needed before implementation plan |
