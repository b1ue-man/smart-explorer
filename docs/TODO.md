...
| 21c | **Peer remote execution** — validated Share exec requests deliberately return `Unsupported`; the security/integration plan is in [`REMOTE_EXECUTION_PLAN.md`](REMOTE_EXECUTION_PLAN.md) | 🚧 | implement only with default-deny per-device grants, a separate authenticated Exec ALPN, atomic revoke/start semantics, streaming I/O, Windows Job Objects, Linux transient systemd cgroups, and installed Windows↔Linux E2E; argv and shell modes intentionally remain unrestricted and share one full-user-code permission |
