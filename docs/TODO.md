...
| # | Item | State | Notes |
|---|---|---|---|
| D1 | **Remote drive real-device certification** — current source can expose one selected saved remote, Google Drive root, or concrete Share target as a real Dokany drive (not CfAPI), with live daemon-side Peer probing, exact-root connection leases, strict-or-explicit-trusted root admission, RO-by-default write-capability gating, durable whole-file recovery and a pinned automatic Dokany 2.3.1.1000 installer path | 🚧 | live SSH reachability/key/Agent-v9 `--serve-root` was proven on 2026-07-21, but this still needs one real Windows + official Dokany 2.3.1.1000 run proving DLL API 231 and kernel-driver protocol 0x190/400, then covering Explorer and Obsidian repeated truncate/write/flush/close plus temp-file→atomic-replace, conflict/retry, connection-renewal/remount and eject; GNU cross-compilation and the task suite do not close this hardware/driver smoke item |
