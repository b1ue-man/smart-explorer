...
| A5 | **Remote storage analysis** ✅ 0.5.61: `analytics::scan_backend` walks any VFS backend (SFTP/FTP/WebDAV/Drive) into the same `SizeNode` tree; the overlay auto-scans the current remote folder when browsing a remote, with a "📡 <conn>" scan button. Serial (one `list_dir` round-trip/dir) so slower than local; reuses the cached backend (cap-bounded). Drill/treemap/reveal all work on the remote path. Future: parallel listing for `parallelism()>1` backends (Drive). | ✅ | 0.5.61 |
