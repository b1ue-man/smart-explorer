...
| # | Item | State | Notes |
|---|---|---|---|
| D1 | **Remote drive real-device certification** — current source can expose one selected saved remote, Google Drive root, or Share target as a real Dokany drive (not CfAPI), with strict-or-explicit-trusted root admission, RO-by-default write-capability gating and durable whole-file recovery | 🚧 | still needs one real Windows + official Dokany 2.3.1/API 231 run covering Explorer and Obsidian repeated truncate/write/flush/close plus temp-file→atomic-replace, conflict/retry, reconnect and eject; GNU cross-compilation and the task suite do not close this hardware/driver smoke item |
