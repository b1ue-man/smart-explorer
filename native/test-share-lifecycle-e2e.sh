...
# ─── Rooms: invite code, membership, file transactions, concurrency ────────
# C creates a Room and D joins it with nothing but the printed invite code.
# Both workers already run against the working signaling server. Every later
# target path is derived from prior CLI output (identity, status, code).
...
test -f "$export_c/nested/tree/renamed.txt" && test ! -e "$export_c/nested/tree/one.txt"
