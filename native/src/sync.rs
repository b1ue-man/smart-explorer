...
fn parent_of(path: &str) -> Option<String> {
    ...
    t.rfind('/').map(|i| if i == 0 { "/".to_string() } else { t[..i].to_string() })
}
...
mod tests {
    ...
    fn mirrors_tree_and_updates_changed() {
        ...
        let (tx, rx) = unbounded();
        start_sync(sb, fwd(&src), db, fwd(&dst), SyncOptions { delete_extra: false, dry_run: false }, tx);
        let r = wait(&rx);
    ...
    fn delete_extra_removes_orphans() {
        ...
        let (sb, db) = handles(&src, &dst);
        let (tx, rx) = unbounded();
        start_sync(sb, fwd(&src), db, fwd(&dst), SyncOptions { delete_extra: true, dry_run: false }, tx);
        let r = wait(&rx);
        assert!(dst.join("keep.txt").exists());
        assert!(!dst.join("orphan.txt").exists(), "orphan file must be deleted");
        assert!(!dst.join("gone/z.txt").exists(), "orphan dir contents deleted");
    ...
    fn dry_run_writes_nothing() {
        ...
        let (sb, db) = handles(&src, &dst);
        let (tx, rx) = unbounded();
        start_sync(sb, fwd(&src), db, fwd(&dst), SyncOptions { delete_extra: false, dry_run: true }, tx);
        let r = wait(&rx);
        assert_eq!(r.stats.copied, 1);
