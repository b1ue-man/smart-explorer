...
pub(super) fn run_upload_plan(
    ...
    let plan = match plan {
        Ok(plan) => plan,
        Err(error) => {
            if error != super::cancel::CANCELED_ERROR {
                errors.push(error);
            }
    ...
    for directory in &plan.dirs {
        ...
        if !seen_dirs.insert(directory.as_str()) {
            continue;
        }
        let path = rjoin(destination, directory);
