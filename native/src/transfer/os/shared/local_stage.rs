//! Local download staging: a private sibling `.part` file, a free-space
//! preflight, and publication only after the full byte count arrived.
use std::path::{Path, PathBuf};

/// Headroom kept free on the local volume beyond the expected download size.
const DOWNLOAD_SPACE_MARGIN_BYTES: u64 = 32 * 1024 * 1024;

pub(super) fn create_download_part(dest: &Path) -> std::io::Result<(PathBuf, std::fs::File)> {
...
pub(super) fn cleanup_partial(path: &Path) {
    let _ = std::fs::remove_file(path);
}

pub(super) fn ensure_local_space(dest: &Path, expected_bytes: u64) -> Result<(), String> {
    if expected_bytes == 0 {
        return Ok(());
    }
    let needed = expected_bytes.saturating_add(DOWNLOAD_SPACE_MARGIN_BYTES);
    if let Some(free) = super::platform::available_space_for_path(dest) {
        if free < needed {
            return Err(format!(
                "Nicht genug lokaler Speicher fuer den Temp-Download: benoetigt ca. {}, frei {}",
                crate::format::format_bytes(needed),
                crate::format::format_bytes(free)
            ));
        }
    }
    Ok(())
