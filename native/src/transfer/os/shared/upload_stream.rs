...
fn observation(metadata: &Metadata, path: &Path) -> Result<Observation, String> {
    ...
    Ok(Observation {
        ...
        modified: metadata
            .modified()
            .map_err(|error| format!("{}: Änderungszeit lesen: {error}", path.display()))?,
...
impl UploadSource {
    pub(super) fn open(path: &Path) -> Result<Self, String> {
        let before = observation(
            &std::fs::symlink_metadata(path)
                .map_err(|error| format!("{}: Quelle prüfen: {error}", path.display()))?,
            path,
        )?;
        let file = File::open(path)
            .map_err(|error| format!("{}: Quelle öffnen: {error}", path.display()))?;
        let source = Self {
            file,
    ...
    pub(super) fn verify(&self) -> Result<(), String> {
        let opened = observation(
            &self.file.metadata().map_err(|error| {
                format!("{}: Geöffnete Quelle prüfen: {error}", self.path.display())
            })?,
            &self.path,
        )?;
        let named = observation(
            &std::fs::symlink_metadata(&self.path).map_err(|error| {
                format!("{}: Quellpfad erneut prüfen: {error}", self.path.display())
            })?,
            &self.path,
        )?;
    ...
    pub(super) fn copy_to(
        ...
        if copied != expected {
            return Err(format!(
                "{}: Quelle unvollständig gelesen ({copied} von {expected} Bytes)",
                self.path.display()
            ));
        }
        self.verify()
