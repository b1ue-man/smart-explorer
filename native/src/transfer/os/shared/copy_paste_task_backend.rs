...
impl TaskBackend {
    pub(super) fn new(root: &Path) -> Self {
        Self {
            ...
            cancel_after_promote: None,
            mutate_source: None,
            bulk_mismatch: false,
            bulk_error: false,
            mutations: AtomicUsize::new(0),
            read_calls: AtomicUsize::new(0),
            replacing_writes: AtomicUsize::new(0),
...
impl Backend for TaskBackend {
    ...
    fn open_write(&self, _path: &str) -> VfsResult<Box<dyn Write + Send>> {
        ...
        Err(io::Error::other(
            "copy must not fall back to a replacing writer",
        ))
    }
        ...
        match self.stage_fault {
            StageFault::Unsupported => {
                return Err(io::Error::new(
                    ...
                    "fixture exclusive writer unavailable",
                ))
            }
            StageFault::ForeignOnOpen => {
                ...
                return Err(io::Error::new(
                    ...
                    "foreign stage appeared",
                ));
            }
            StageFault::None | StageFault::ForeignOnFlush => {}
        }
        Ok(Box::new(StageWriter {
            inner: Some(self.inner.open_write_new(path)?),
    ...
    fn promote_copy_stage(&self, stage: &str, destination: &str) -> VfsResult<()> {
        ...
        if let Some(cancel) = &self.cancel_after_promote {
    ...
    fn copy_file(&self, _source: &str, _destination: &str) -> VfsResult<u64> {
        ...
        Err(io::Error::other(
            "GUI copy must not use replacing server-copy",
        ))
    }
    ...
    fn get_tree(&self, source: &str, destination: &Path) -> VfsResult<u64> {
        ...
        self.bulk_destinations
            .lock()
            .unwrap()
            .push(destination.to_path_buf());
        let count = copy_tree(Path::new(source), destination)?;
        if self.bulk_error {
            return Err(io::Error::other("ambiguous bulk fixture failure"));
        }
    ...
    fn put_tree(&self, _source: &Path, _destination: &str) -> VfsResult<u64> {
        ...
        Err(io::Error::other(
            "copy must not dispatch final-tree bulk upload",
        ))
    }
}
