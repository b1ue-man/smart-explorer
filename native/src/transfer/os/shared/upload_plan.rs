...
pub(super) struct UploadPlan {
    ...
    /// Protected omissions (the active app trash) left out of the plan.
...
pub(super) fn collect_paths(
    ...
    for path in paths {
        ...
        let base = source
            ...
            .ok_or_else(|| format!("{}: ungültiger Dateiname", source.display()))?;
        if crate::apptrash::excluded_name(base) {
...
fn collect(
    ...
    super::cancel::check(cancel)?;
    if super::platform::upload_is_link_like(&metadata) {
        return Err(format!(
