...
use super::direct_ledger::{
    DirectLedgerError, DirectRequestDirection, DirectRequestEntry, DirectRetryState,
    MAX_DIRECT_REQUEST_ENTRIES,
};
...
impl DirectRequestEntry {
    fn validate_persisted(&self) -> Result<(), DirectLedgerError> {
        ...
        validate_retry(&self.retries.request)?;
        validate_retry(&self.retries.request_receipt)?;
        validate_retry(&self.retries.decision)?;
        validate_retry(&self.retries.decision_receipt)?;
...
fn validate_retry(retry: &DirectRetryState) -> Result<(), DirectLedgerError> {
