...
fn register(dirs: &mut HashMap<String, Vec<VfsMeta>>, path: &str, is_dir: bool, size: u64, mtime_ms: i64) {
...
impl Backend for ZipBackend {
    ...
    fn list_dir(&self, path: &str) -> VfsResult<Vec<VfsMeta>> {
        match self.dirs.get(&Self::key(path)) {
            Some(v) => Ok(v.clone()),
            None => Err(io::Error::new(io::ErrorKind::NotFound, "Ordner nicht im Archiv")),
        }
    }
...
fn readonly() -> io::Error {
    io::Error::new(io::ErrorKind::PermissionDenied, "ZIP ist schreibgeschützt — zum Bearbeiten entpacken")
}
...
mod tests {
    ...
    fn browse_and_extract() {
        ...
        let mut buf = Vec::new();
        be.open_read("/a.txt").unwrap().read_to_end(&mut buf).unwrap();
