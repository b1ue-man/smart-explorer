...
impl App {
    ...
    pub(in crate::app) fn ui_commandbar(&mut self, ui: &mut egui::Ui, wide: bool) {
        if ui
            .toggle_value(&mut self.recursive, "Rekursiv")
            .on_hover_text(
                "Unterordner durchsuchen (Ctrl+R) · mit aktivem Filter werden nur Treffer behalten",
            )
            .changed()
        ...
        ui.menu_button("Verbindung", |ui| {
            ...
            egui::ScrollArea::vertical()
                .max_height(360.0)
                .show(ui, |ui| self.ui_menu_connect(ui));
        ...
        ui.menu_button("Sync", |ui| {
            ...
            egui::ScrollArea::vertical()
                .max_height(420.0)
                .show(ui, |ui| self.ui_menu_sync(ui));
        });
    ...
    fn ui_file_menu(&mut self, ui: &mut egui::Ui) {
        ...
        for (label, cut) in [
            ("Kopieren   Ctrl+C", false),
            ("Ausschneiden   Ctrl+X", true),
        ] {
        ...
        if ui
            .add_enabled(
                !self.root_path.is_empty(),
                egui::Button::new("Einfügen   Ctrl+V"),
            )
            .clicked()
        {
        ...
        let (label, tip) = match disposition {
            Some(crate::vfs::DeleteDisposition::Permanent) => (
                "Löschen…",
                "Server löscht endgültig; Bestätigung erforderlich",
            ),
            Some(crate::vfs::DeleteDisposition::Unsupported) => {
                ("Löschen", "Diese Quelle ist schreibgeschützt")
            }
            _ => (
                "In Papierkorb",
                "Ausgewählte Einträge in den Papierkorb verschieben",
            ),
        };
        if ui
            .add_enabled(
                selected && disposition != Some(crate::vfs::DeleteDisposition::Unsupported),
    ...
    fn ui_view_menu(&mut self, ui: &mut egui::Ui) {
        if ui
            .selectable_label(self.split, "Zwei Bereiche   F6")
            .clicked()
        {
            self.toggle_split();
            ui.close_menu();
        ...
        if ui
            .checkbox(&mut self.show_summary, "Zusammenfassung")
            .changed()
        {
            self.save_ui_state();
        }
        if ui
            .checkbox(&mut self.appearance.detailed_columns, "Zusätzliche Spalten")
            .on_hover_text("Pfad, Erstelldatum und Verzeichnistiefe anzeigen")
            .changed()
        {
            self.save_ui_state();
        }
        if ui
            .checkbox(&mut self.appearance.compact, "Kompakte Dateizeilen")
            .changed()
        {
            self.save_ui_state();
        }
        ...
        if ui.checkbox(&mut self.dirs_first, "Ordner zuerst").changed() {
            if !self.root_path.is_empty() {
                self.dir_sort
                    .insert(self.location_key(&self.root_path), self.dirs_first);
        ...
        if ui
            .selectable_label(self.show_analytics, "Speicheranalyse…")
            .clicked()
        {
