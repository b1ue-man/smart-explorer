...
pub(crate) fn run_analysis_window(request: Result<AnalysisStartup, String>) -> eframe::Result<()> {
    let options = eframe::NativeOptions {
        viewport: egui::ViewportBuilder::default()
            ...
            .with_title("Smart Explorer – Administrator-Speicheranalyse"),
        ..Default::default()
    };
    eframe::run_native(
...
impl AnalysisWindow {
    fn new(request: Result<AnalysisStartup, String>) -> Self {
        ...
        match request.and_then(|request| {
            ...
            Err(error) => {
                window.outcome = Some(ScanOutcome::failed("Administrator-Analyse", error))
            }
        }
        window
    }
    ...
    fn start(&mut self) {
        if !self.admitted || self.rx.is_some() {
            return;
        }
    ...
    fn poll(&mut self) {
        match self.rx.as_ref().map(|rx| rx.try_recv()) {
            ...
            Some(Err(crossbeam_channel::TryRecvError::Disconnected)) => {
                self.rx = None;
                self.outcome = Some(ScanOutcome::failed(
                    &self.root,
                    "Analyse-Thread ohne Ergebnis beendet",
                ));
            }
            _ => {}
        }
...
impl eframe::App for AnalysisWindow {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        let base = if self.focus.is_empty() {
            self.root.clone()
        } else {
            format!(
                "{}/{}",
        ...
        let node = focused(
            self.outcome.as_ref().and_then(|o| o.tree.as_ref()),
            &self.focus,
        );
