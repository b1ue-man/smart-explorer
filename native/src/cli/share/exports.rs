...
fn add_export(args: ExportAddArgs) -> Result<(), String> {
    ...
    let config = export_config_mut(&mut candidate, args.scope.room.as_deref())?;
...
fn remove_export(args: ExportRemoveArgs) -> Result<(), String> {
    ...
    let removed = config.roots.remove(matches[0]);
    profiles.persist_replacement(candidate)?;
    println!("Removed export {}{}", removed.path, super::refresh_note());
    Ok(())
}
