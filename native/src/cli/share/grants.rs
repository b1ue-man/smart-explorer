...
struct DeleteArgs {
    #[arg(
        ...
        help = "Device id, device name, fingerprint, or request id of the authorization; omit when exactly one grant exists"
    )]
...
pub(super) fn run(args: GrantsArgs) -> Result<(), String> {
    match args.command {
        ...
        Some(GrantsCommand::Delete(command)) => delete(command, args.json),
        Some(GrantsCommand::Removed(command)) => removed(command, args.json),
        Some(GrantsCommand::Exec(command)) => grants_exec::run(command, args.json),
    }
}

fn delete(args: DeleteArgs, json: bool) -> Result<(), String> {
    ...
    let home = Some(super::default_home());
...
fn removed(args: RemovedArgs, json: bool) -> Result<(), String> {
